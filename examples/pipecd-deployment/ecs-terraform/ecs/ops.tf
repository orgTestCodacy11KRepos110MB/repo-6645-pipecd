    ...
    [
      {
        ...
        command = [
          "/bin/sh -c 'apk add aws-cli; aws s3 cp s3://namba-pipecd-control-plane-config/control-plane-config.yaml ./; sed -i -e s/pipecd-mysql/${var.db_instance_address}/ control-plane-config.yaml; echo $ENCRYPTION_KEY >> encryption-key; pipecd ops --cache-address=${var.redis_host}:6379 --config-file=control-plane-config.yaml --log-encoding=humanize --metrics=true;'"
        ]
        entrypoint = [
          "sh",
          "-c"
